...
printf '%s %s\n' "$INPUT_REMOTE_HOST" "$INPUT_SSH_PUBLIC_KEY" >> /etc/ssh/ssh_known_hosts

echo "Connecting to $INPUT_REMOTE_HOST..."
docker --log-level debug --host $INPUT_REMOTE_HOST login --username=$INPUT_USERNAME --password=$INPUT_PASSWORD $INPUT_REGISTRY
echo "Pulling image"
docker --log-level debug --host $INPUT_REMOTE_HOST pull $INPUT_REGISTRY/$INPUT_IMAGE:$INPUT_TAG 2>&1
